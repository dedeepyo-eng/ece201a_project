...
idle_time_sum = 0.0  # return for roofline only
...
def run_GEMM(
    ...
    if TC.kp1 == 1 and TC.kp2 == 1:  # no parallelism
        forward_time, forward_idle_time, _, _ = TC.getGEMMTime(TC.M, TC.K, TC.N, "GEMM")
    elif TC.t == "CR":
    ...
    if getattr(TC.model, "backward", False):
        if TC.kp1 == 1 and TC.kp2 == 1:
            grad_act_time, act_idle_time, _, _ = TC.getGEMMTime(TC.M, TC.N, TC.K, "Cb_act")
            grad_wt_time, wt_idle_time, _, _ = TC.getGEMMTime(TC.K, TC.M, TC.N, "Cb_wt")
            backward_time = grad_act_time + grad_wt_time
            backward_idle_time = act_idle_time + wt_idle_time
        elif TC.t == "CR":
    ...
    idle_time = forward_idle_time + backward_idle_time
    total_time = forward_time + backward_time + dp_reduction_time
    idle_fraction = idle_time / total_time if total_time > 0 else 0.0
...
def run_LLM(
    ...
    return _run_llm_llmtest(exp_hw_config, exp_model_config, exp_dir, mode)
