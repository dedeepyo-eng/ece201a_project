...
class Chip:
    ...
    def __init__(self, filename = None, etree = None, parent_chip = None, wafer_process_list = None, assembly_process_list = None, test_process_list = None, layers = None, ios = None, adjacency_matrix_definitions = None, average_bandwidth_utilization = None, block_names = None, static = False, variable_dict=None) -> None:
        ...
        self.wafer_process = self.find_wafer_process(self.resolve_value(variable_dict, attributes["wafer_process"]), wafer_process_list)
        self.assembly_process = self.find_assembly_process(self.resolve_value(variable_dict, attributes["assembly_process"]), assembly_process_list)
        self.test_process = self.find_test_process(self.resolve_value(variable_dict, attributes["test_process"]), test_process_list)
        self.stackup = self.build_stackup(self.resolve_value(variable_dict, attributes["stackup"]), layers)

        # Recursively handle the chips that are stacked on this chip.
        self.chips = []
        # --- Floorplan/floorplan_dict support for child chip instancing ---
        ...
        if floorplan is not None and floorplan.strip() != "" and floorplan_dict is not None and floorplan_dict.strip() != "":
            ...
            for entry in dict_entries:
                if ":" in entry:
                    ...
                    char_to_chip[k.strip()] = v
            for char, chip_name in char_to_chip.items():
                n_instances = fp_counts.get(char, 0)
                for child in root:
            ...
            for chip_def in root:
                self.chips.append(Chip(etree=chip_def, parent_chip=self, wafer_process_list=wafer_process_list, assembly_process_list=assembly_process_list, test_process_list=test_process_list, layers=layers, ios=ios, adjacency_matrix_definitions=adjacency_matrix_definitions, average_bandwidth_utilization=average_bandwidth_utilization, block_names=block_names, static=static, variable_dict=variable_dict))
        ...
        bb_cost = self.resolve_value(variable_dict, attributes["bb_cost"])
        if bb_cost == "":
            self.bb_cost = None
        else:
            self.bb_cost = float(bb_cost)
        bb_quality = self.resolve_value(variable_dict, attributes["bb_quality"])
        if bb_quality == "":
        ...
        aspect_ratio = self.resolve_value(variable_dict, attributes["aspect_ratio"])
        if aspect_ratio == "":
            self.aspect_ratio = 1.0
        else:
            self.aspect_ratio = float(aspect_ratio)

        x_location = self.resolve_value(variable_dict, attributes["x_location"])
        if x_location == "":
            self.x_location = None
        else:
            self.x_location = float(x_location)

        y_location = self.resolve_value(variable_dict, attributes["y_location"])
        if y_location == "":
            self.y_location = None
        else:
            self.y_location = float(y_location)
        ...
        self.gate_flop_ratio = float(self.resolve_value(variable_dict, attributes["gate_flop_ratio"]))
        ...
        self.reticle_share = float(self.resolve_value(variable_dict, attributes["reticle_share"]))

        # All NRE costs scale with the quantity of chips produced.
        self.quantity = int(self.resolve_value(variable_dict, attributes["quantity"]))

        # If a chip is defined as buried (such as a bridge die in EMIB), it is not counted in the total area of stacked chips.
        self.buried = self.resolve_value(variable_dict, attributes["buried"])
        ...
        self.power = float(self.resolve_value(variable_dict, attributes["power"]))
        self.core_voltage = float(self.resolve_value(variable_dict, attributes["core_voltage"]))
