...
# GEORGE KARFAKIS NOV 19.
# rearrange.py but with multicore support.
# didn't work, code here doesn't run but you may want to look at it. We can probably speed up by a lot doing this, we have 128 cores.
...
def process_chunk(chunk, all_boxes, inflation=0):
    overlaps = []
    for box in chunk:
        # Compute the inflated bounds for the current box
        start_x = box.start_x - inflation
        start_y = box.start_y - inflation
        end_x = box.end_x + inflation
        end_y = box.end_y + inflation

        # Check overlaps with all boxes
        for other_box in all_boxes:
            if box == other_box:
                continue
            # Compute the inflated bounds for the other box
            o_start_x = other_box.start_x - inflation
            o_start_y = other_box.start_y - inflation
            o_end_x = other_box.end_x + inflation
            o_end_y = other_box.end_y + inflation

            # Check overlap
            if not (o_end_x < start_x or end_x < o_start_x or o_end_y < start_y or end_y < o_start_y):
                overlaps.append((box, other_box))

    return overlaps

def check_all_overlaps(boxes, inflation=0, max_workers=128):
    # num_workers = min(mp.cpu_count()//2, max_workers, len(boxes))
    num_workers = 1

    # Sort boxes by start_x
    sort_boxes = sorted(boxes, key=lambda box: box.start_x)

    # Divide the boxes into chunks for parallel processing
    chunk_size = len(sort_boxes) // num_workers
    chunks = [sort_boxes[i:i + chunk_size] for i in range(0, len(sort_boxes), chunk_size)]

    # Use multiprocessing to process each chunk
    with mp.Pool(processes=num_workers) as pool:
        results = pool.starmap(process_chunk, [(chunk, sort_boxes, inflation) for chunk in chunks])

    # Combine all overlaps from the chunks
    combined_overlaps = [overlap for result in results for overlap in result]

    # Remove duplicates (as overlaps will be counted twice for each pair)
    unique_overlaps = set(tuple(sorted(pair, key=lambda x: x.name)) for pair in combined_overlaps)

    return list(unique_overlaps)
